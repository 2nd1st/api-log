...
func TestAppendOneWritesJSONLLine(t *testing.T) {
	...
	w := New(dir, 16, nil, nil, func() time.Time { return fixed })
	stop := w.Start()
...
func TestNoAuthGoesToZeroHashFile(t *testing.T) {
	dir := t.TempDir()
	w := New(dir, 4, nil, nil, nil) // clock = time.Now
	stop := w.Start()
