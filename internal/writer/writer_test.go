...
func TestAppendOneWritesJSONLLine(t *testing.T) {
	...
	w := New(dir, 16, nil, nil, nil, nil, nil, func() time.Time { return fixed })
	stop := w.Start()
...
func TestNoAuthGoesToZeroHashFile(t *testing.T) {
	dir := t.TempDir()
	w := New(dir, 4, nil, nil, nil, nil, nil, nil) // clock = time.Now
	stop := w.Start()
...
func TestDailyRotationGzipsOldFile(t *testing.T) {
	...
	w := New(dir, 16, nil, nil, nil, nil, nil, clock)
	stop := w.Start()
...
func TestTrySendDropsWhenFull(t *testing.T) {
	...
	w := New(dir, 1, nil, nil, nil, nil, nil, nil)
...
func TestStopIdempotent(t *testing.T) {
	w := New(t.TempDir(), 4, nil, nil, nil, nil, nil, nil)
	stop := w.Start()
	stop()
