...
完整流程——service 用户、env 文件、data 目录、带 hardening 默认值的 systemd unit——见 [`deploy/systemd/`](./deploy/systemd/)。Caddy + nginx 把 TLS 套在 read API + viewer SPA 同一个域名上的示例见 [`deploy/reverse-proxy/`](./deploy/reverse-proxy/)。
...
2. **`data/index.sqlite`** —— read API 需要的派生列（status、model、token 数、session 关联、`jsonl_path` + `jsonl_offset`）。可删；秒级从第一层重建。WAL 模式，连接池 10（v0.1.1）。

当 JSONL 与 SQLite 不一致时，JSONL 为准。

#### Retention（v0.1.1）

默认关闭。通过 `PUT /api/config/retention` 开启（写入 `runtime_overrides.json`，重启保留）：
...
两个阈值都为 `0`：引擎仍运行（维护 inventory + `/healthz.storage`），但永不删除。详见 [docs/retention.md](./docs/retention.md)：阈值、淘汰顺序、以及防止 retention 与 writer 竞争的 lease 语义。
...
api-log **不内嵌 HTML viewer**。`GET /` 返回一个 JSON 指针，指向独立的 [api-log-viewer](https://github.com/2nd1st/api-log-viewer) 项目；二进制里没有任何 HTML。

## Bundled viewer

api-log 默认在 `/viewer/` 托管 viewer。首次请求时，后端从 [`api-log-viewer`](https://github.com/2nd1st/api-log-viewer) 的某个固定 release 拉取 `dist.zip`，对照烧进 backend binary 的 SHA-256 常量校验该 asset，然后把 bundle 解压到 `data/viewer-cache/` 下的缓存目录。后续请求直接从缓存读。

SHA-256 不匹配对该路由是致命的——`/viewer/` 返回 503，binary 把不匹配写进日志。后端从不提供未校验的 asset。

覆盖开关（env 或 YAML）：

| Knob | 默认 | 效果 |
|---|---|---|
| `APILOG_VIEWER_ENABLED` | true | 设为 false 则不托管；`/viewer/` 返回 503 `disabled`。 |
| `APILOG_VIEWER_REPO` | `2nd1st/api-log-viewer` | 可指向任何 GitHub 仓库。 |
| `APILOG_VIEWER_VERSION` | 与 backend release 绑定 | 要拉取的 tag。 |
| `APILOG_VIEWER_SHA256` | 与 backend release 绑定 | 覆盖 REPO 或 VERSION 时必须同时提供；不匹配返回 503。 |
| `APILOG_VIEWER_LOCAL_PATH` | （未设） | 指向某个 `dist/` 目录的绝对路径。完全跳过 fetch + 校验；适用于离线 / 隔离网络部署，以及本地 viewer 开发。 |
| `APILOG_VIEWER_CACHE_DIR` | `<data_dir>/viewer-cache` | 缓存根目录。 |
| `APILOG_VIEWER_PUBLIC_PATH` | `/viewer` | URL 前缀。 |

后端从不自动更新已缓存的 viewer——fetch 在每个 (repo, version, sha) 三元组下只发生一次。要前滚版本，要么发新的 backend release（同时 bump 这两个常量），要么显式覆盖 `APILOG_VIEWER_VERSION` + `APILOG_VIEWER_SHA256`。
...
- [x] **v0.1 viewer** —— [api-log-viewer](https://github.com/2nd1st/api-log-viewer) —— 多实例聚合、session 树可视化、tool-call 渲染、SSE 重放。
